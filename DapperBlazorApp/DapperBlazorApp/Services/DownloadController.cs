...
namespace DapperBlazorApp.Services
{
    public partial class ExportController : ControllerBase
    {
        ...
        public ExportController(IUserService service)
        {
            _UserService = service;
        }

        [HttpGet("/export/(fileName='{fileName}')")]
        public async Task<FileStreamResult> ExportToExcel(string fileName = null)
        {
            return ToExcel(ApplyQuery(await _UserService.GetUsersAsync(), Request.Query), fileName);
        }
